...
function connect(locals) {
    ...
    const uri = `mongodb+srv://dbrufio:${process.env.DB_PASSWORD}@cluster0.zzn5p.mongodb.net/demo?retryWrites=true&w=majority`
    ...
    return client.connect()
        .then((connection) => {
            ...
            locals.collectionSpells = connection.db('demo').collection('spellbook')
        })
        .catch(err => {
            console.log(err)
            process.exit()
